...
export interface SessionState {
  ...
  uploadedDriver: File | null; // Supporto file XDF / A2L
  maps: ParsedMap[];
...
export const useSession = create<SessionState>((set, get) => ({
  ...
  loadFile: async (file, family, driverFile) => {
    ...
    try {
      ...
      if (driverFile) {
        formData.append('driver', driverFile);
      }
      
      const res = await fetch('/api/parse-map', { method: 'POST', body: formData });
      if (!res.ok) throw new Error('Fallito il parsing della mappa iniziale tramite driver');
      const firstParsedMap: ParsedMap = await res.json();

      const loadedMaps: ParsedMap[] = [firstParsedMap];

      // Se non si è caricato un driver personalizzato, carica il pacchetto di default
      if (!driverFile) {
        ...
        for (const mapId of mapList) {
          ...
          if (mapRes.ok) {
            loadedMaps.push(await mapRes.json());
          }
        }
      }
      ...
      set((s) => ({
        ...
        log: [
          ...
          driverFile ? `[DRIVER] Caricato driver dinamico: ${driverFile.name}` : `[DRIVER] Caricato Map Pack integrato standard.`,
          `[PARSE] Rilevate e mappate ${loadedMaps.length} aree di taratura con assi fisici.`,
      ...
      set((s) => ({
        status: 'error',
        log: [...s.log, `[ERRORE] Caricamento fallito: ${e.message}`]
      }));
    }
  },

  setActiveMap: async (id) => {
    set({ activeMapId: id, selectedKeys: new Set(), pendingDeltas: new Map() });
  },
  ...
  applyMath: (op) => {
    ...
    for (const key of selectedKeys) {
      ...
      next.set(key, { col, row, newPhysical });
    }
  ...
  applyInterpolate: () => {
    ...
    const q11 = getValue(minC, minR);
    const q21 = getValue(maxC, minR);
    const q12 = getValue(minC, maxR);
    const q22 = getValue(maxC, maxR);
    ...
    set({ pendingDeltas: next, log: [...get().log, `[CALIBRATION] Interpolazione 2D applicata su area ${minC},${minR} -> ${maxC},${maxR}`] });
  },
  ...
  applyStage1EGR: () => {
    ...
    if (!egrMap) {
      set({ log: [...log, '[WARNING] Mappa EGR non rilevata. Azione ignorata.'] });
      return;
    }
    const next = new Map<string, CellDelta>();
    for (const cell of egrMap.cells) {
      next.set(`${cell.col},${cell.row}`, { col: cell.col, row: cell.row, newPhysical: 0.0 });
    }
    set({
      activeMapId: egrMap.mapId,
      pendingDeltas: next,
      log: [...log, '[TUNING-STAGE1] EGR Switch chiusa a 0.0% (Chiusura Software EGR).']
    });
  },
  ...
  applyHardcutLimit: () => {
    ...
    if (!torqueMap) {
      set({ log: [...log, '[WARNING] Mappa limitatore di coppia non rilevata. Azione ignorata.'] });
      return;
    }
    const next = new Map<string, CellDelta>();
    for (const cell of torqueMap.cells) {
      ...
      if (isMaxRPM) {
        ...
        const stage1Val = cell.col > 1 && cell.col < 8 ? Math.round(current * 1.15) : current;
        next.set(`${cell.col},${cell.row}`, { col: cell.col, row: cell.row, newPhysical: stage1Val });
      }
    }
    set({
      activeMapId: torqueMap.mapId,
      pendingDeltas: next,
      log: [...log, '[TUNING-STAGE1] Applicato Hardcut Limiter popcorn & Incremento Coppia +15%.']
    });
  },
  ...
  commitPatch: async () => {
    ...
    try {
      const reqBody = {
        mapId: activeMapId,
        deltas: Array.from(pendingDeltas.values())
      };
      ...
      if (data.success && data.patchedBinary) {
        const updatedMaps = maps.map(m => {
          if (m.mapId !== activeMapId) return m;
          return {
            ...m,
            cells: m.cells.map(c => {
      ...
      set({
        status: 'error',
        log: [...log, `[ERRORE EXPORT] Impossibile esportare il file: ${e.message}`]
      });
    }
  },
